...
def transfer(_amount:uint256, _to:address):
    ...
    log Transfer(owner=msg.sender, to=_to, amount=_amount)
...
def approve(_spender:address, _amount:uint256):
    ...
    log Approval(spender=_spender, amount=_amount)
...
def transferFrom(_owner:address, _to:address, _amount:uint256):
    ...
    log Transfer(owner=_owner, to=_to, amount=_amount)
...
def _mint(_amount: uint256, _to: address):
    ...
    log Transfer(owner=empty(address), to=_to, amount=_amount)
